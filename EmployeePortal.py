...
def time_in():
    emp_status = 0
    remarks = ''
    message='User Clocking'
    if request.method == 'POST':
        timeInMessage = empClass.userTimeIn(emp_status, remarks,message, current_user)
    return render_template('payroll.html',message=timeInMessage)

# TO DO TIME IN FUNCTION FIX CONDITIONS ERROR INCORRECT EMPLOYEE STATUS AND ADD VALIDATOR TO CHECK IF USER HAS TIME IN TODAY AND GRAVEYARD SHIFT
# TIME OUT FUNCTION
# CALENDAR MODULE
# EDIT RESTDAY MODULE
